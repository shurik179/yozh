...
while True:
    for i in range (1,5):
        target = start_yaw+90*i
        bot.turn_to(target, CW)
        angle = bot.normalize(bot.IMU_yaw()-start_yaw)
        bot.set_text(1, "Yaw:   {:.1f}".format(angle),font = FONT_BOLD)
        error = bot.normalize(bot.IMU_yaw()-target)
        bot.set_text(3, "Error: {:.1f}".format(error),font = FONT_BOLD)
        time.sleep(3)
    for i in range (1,5):
        target = start_yaw-90*i
        bot.turn_to(target, CCW)
        angle = bot.normalize(bot.IMU_yaw()-start_yaw)
        bot.set_text(1, "Yaw:   {:.1f}".format(angle),font = FONT_BOLD)
        error = bot.normalize(bot.IMU_yaw()-target)
        bot.set_text(3, "Error: {:.1f}".format(error),font = FONT_BOLD)
