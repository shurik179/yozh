...
PIDcontroller CourseController;
    ...
    switch (*motorMode){
        ...
        */
        case MOTOR_MODE_DRIVE:
            //FIXME!!!!
            //set up the PID controllers for course and speed control
            //speed controller will take as input headign difference (in degrees) and outputs a value between -1..1 - it will be later multiplied by MOTOR_MAX_POWER
            float Kp= (*motorKp)*0.001; //10^3
            ...
            CourseController.configure(Kp,Kp/Ti,Kp*Td,Ilim);
            CourseController.setTarget(0.0f);
...
void updateSpeed(){
    ...
    if (*motorMode == MOTOR_MODE_DRIVE) {
        ...
        int16_t powerL = motorPower[0];
        int16_t powerR = motorPower[1];
        //now, compute and apply differential correction
        if (CourseController.is_active()) {
            int16_t dYaw=normalizeAngle(*direction-*yaw );
            int16_t diffCorrection=-(int16_t)(CourseController.update(0.1f*dYaw) * MOTOR_MAX_POWER);//positive correction means we need to turn cw
            //Serial.println(diffCorrection);
